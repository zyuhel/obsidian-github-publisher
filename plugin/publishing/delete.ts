	...
	vault: Vault, metadataCache: MetadataCache)
{
	...
	if (localShared.length > 0) {
		...
		for (const file of localShared) {
			if (!allSharedConverted.includes(file.path.trim())) {
				let checkingIndex = false;
				if (file.name == 'index.md') {
					const frontmatter = this.metadataCache.getFileCache(file).frontmatter;
					checkingIndex = frontmatter.index === "true" || frontmatter.autoclean === "false" || !frontmatter.share
				}
				if (!checkingIndex) {
					noticeLog('trying to delete file : ' + file.path, settings);
	...
	noticeLog(deleted + ' files from local folder', settings);
}
